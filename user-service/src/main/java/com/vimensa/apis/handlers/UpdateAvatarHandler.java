...
public class UpdateAvatarHandler extends BaseApiHandler {
    @Override
    ...
    public _BaseResponse hanler(HttpServerRequest request, String body) throws Exception {
        ...
        if(avatar!=null && avatar.length()> 0 && loginSession.getAccessToken()!=null && loginSession.getAccessToken().length()>0 && loginSession.getAppId()!=null && loginSession.getAppId().length()>0){
            response = userService.updateAvatar(avatar,loginSession);
        }else{
